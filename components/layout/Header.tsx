...
export function Header() {
  ...
  return (
    <>
      <motion.header
        style={{ y: headerY }}
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 overflow-visible isolate ${
          isScrolled
      ...
      >
        ...
        {/* مهم: مايبقاش overflow-hidden عشان مايقصش Dropdown اللغة */}
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative">
          <div className="flex items-center justify-between h-20">
            ...
            <nav
              className={`hidden md:flex items-center relative perspective-container ${
                t.nav.services === 'الخدمات' ? 'space-x-reverse space-x-1' : 'space-x-1'
              }`}
            >
            ...
            {/* مهم: خليها z أعلى عشان Dropdown يطلع فوق الهيدر */}
            <div className="flex items-center gap-3 relative z-50">
              ...
              >
                <motion.div className="absolute inset-0 bg-gradient-to-br from-primary-500/20 to-cyan-500/20 opacity-0 group-hover:opacity-100 transition-opacity" />
              ...
              <nav className="flex flex-col h-full pt-24 pb-8 px-6">
                {navItems.map((item, index) => (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                  >
                    <motion.div className="absolute inset-0 bg-gradient-to-r from-primary-500/10 to-cyan-500/10 opacity-0 group-hover:opacity-100 transition-opacity" />
                    <span className="relative z-10">{item.label}</span>

                    {/* Glow effect */}
                    <motion.div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-full bg-gradient-to-b from-primary-500 to-cyan-500 opacity-0 group-hover:opacity-100 rounded-r transition-opacity" />
                  </motion.button>
                ))}
              </nav>
            </motion.div>
